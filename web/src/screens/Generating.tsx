...
export default function Generating() {
  const { selfieDataUrl, sceneId, aspectRatio, quality, setResult, setError } = useBooth();

  useEffect(() => {
    if (!selfieDataUrl || !sceneId || !aspectRatio || !quality) {
      setError("Something went wrong — missing photo, scene, or format. Please start again.");
      return;
    }

    const { base64, mimeType } = dataUrlToBase64(selfieDataUrl);

    generatePhoto({ selfieBase64: base64, mimeType, sceneId, aspectRatio, quality })
      .then((res) => {
    ...
    <div className="flex h-full w-full flex-col items-center justify-center gap-8 bg-nalco-navy">
      ...
      <p className="text-white/60">
        {quality === "standard"
          ...
          : "This can take up to a minute — worth the wait for the detail"}
      </p>
    </div>
  );
}
